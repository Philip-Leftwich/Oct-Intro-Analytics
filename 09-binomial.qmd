...
downloadthis::download_link(
  link = "https://github.com/UEABIO/5023B/blob/dev/files/beetles.csv",
...
beetles <- beetles |> 
  ...
  assertr::verify(number_killed <= number_tested, error_fun = just_warn) |> # numbers check
...
Unless the number of trials is identical in every row of data - then binomial models will always have greater power than the equivalent t-tests: 
...
ggplot(power_df, aes(x = p1, y = power, color = method)) +
    ...
    facet_grid(scenario ~ n_rows, labeller = labeller(
        ...
        n_rows = function(x) paste("Observations \n per replicate =","\n", x)
    )) +
    labs(
