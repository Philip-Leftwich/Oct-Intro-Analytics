...
One application of mixed models is to account for overdispersion in fixed-dipsersion models (Poisson and Binomial). If a random effect is included to account for individual-level variation we may find our models have a better fit.
:::
...
This dataset represents results from replicate feeding experiments investigating how *Spodoptera frugiperda* larvae respond to increasing concentrations of a plant defensive compound. The explanatory variable is the concentration of the benzoxazinoid DIMBOA incorporated into the larval diet (mM), while the response variable y is a composite detoxification index combining cytochrome P450 and glutathione S-transferase activity together with expression of a marker detoxification gene.
...
downloadthis::download_link(
  ...
  link = "https://github.com/UEABIO/5023B/blob/dev/files/benzo.csv",
...
Then the difference between a fixed-effect model and a mixed-effect model will be *small*. 
...
ggpredict(mixed_model, terms = "benzo_um") |>  
...
By default the figure now produces **prediction intervals**

```{r}

ggpredict(mixed_model, terms = c("benzo_um", "group"), type = "random") |>  
plot(x = _, show_data = TRUE)
...
emmeans::emmeans(mixed_model, specs = "benzo_um", at = list(x = seq(0,10,2)))
...
ggpredict(mixed_model, terms = "benzo_um", interval = "confidence")
...
```{r}
ggpredict(mixed_model, terms = "benzo_um", interval = "prediction")
